...
class BufferResultsController extends Controller
{
    ...
    public function actionResults()
    {
        ...
        $member_number=Members::find()
            ->where(['uid'=>$user])
            //  ->where(['member_number'=>$member_number['membership_number']])
            ->one();
        $dateOfBirth=$member_number->date_of_birth;
        $c= date('Y');
        $y= date('Y',strtotime($dateOfBirth));
        $age= $c-$y;
        ...
        if ($age < 55 && $type=='1'){
            ...
            Yii::$app->session->setFlash('benefit_success', Yii::t('yii', 'It is not applicable you are bellow retirement age instead test for Projection'));
