...
class UserController extends Controller
{
        ...
        if ($model->load(Yii::$app->request->post()) && $member->load(Yii::$app->request->post())) {
            ...
            if ($model->password_new !=''){
                ...
                if($validateOldPass)
                {
                    ...
                    Yii::$app->session->setFlash('oldpass_fail', Yii::t('yii', 'Current password is Incorrect'));
    ...
    public function actionOther($id)
    {
        ...
        $model = $this->findModel($id);
        $member=Members::find()->where(['uid'=>$id])->one();
        ...
        $member_data=Members::find()
            ->where(['uid'=>Yii::$app->user->identity->getId()])
            ->one();
        ...
        $existingPhoneNumber=$member_data['previous_mobile_numbers'];

        if ($model->load(Yii::$app->request->post()) && $member->load(Yii::$app->request->post())) {

            if (!empty($existingPhoneNumber)){
                $oldNumbers=$existingPhoneNumber.','.$member->mobile_number;
            }

            if (empty($existingPhoneNumber)){
                $oldNumbers=$member->mobile_number;
            }
            ...
            if ($model->password_new !=''){
                ...
                {
                    if ($model->save(false)){
                        Members::updateAll(['mobile_number'=>$member->mobile_number,'username'=>$model->username,'email'=>$model->email,'previous_mobile_numbers'=>$oldNumbers],['uid'=>$model->id]);
                    }
